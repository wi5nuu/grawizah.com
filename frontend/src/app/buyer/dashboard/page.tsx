...
export default function BuyerDashboardPage() {
  ...
  const recentActivity = [
    { action: 'Sent inquiry to PT Nusantara Agro for Virgin Coconut Oil', time: '2h ago', type: 'inquiry', icon: MessageSquare, color: 'text-primary-500' },
    { action: 'Created RFQ for Arabica Coffee Beans - 500MT', time: '5h ago', type: 'rfq', icon: FileText, color: 'text-accent-500' },
    { action: 'Received quotation from Java Spice Trading', time: '1d ago', type: 'quote', icon: CheckCircle2, color: 'text-green-500' },
    { action: 'Uploaded trade compliance documents', time: '2d ago', type: 'document', icon: Lock, color: 'text-amber-500' },
  ];
      ...
      <div className="p-6 lg:p-8 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-gray-500 mt-1">Welcome back, <span className="gradient-text font-medium">{user?.email?.split('@')[0] || 'Buyer'}</span></p>
          </div>
          <div className="flex items-center gap-3">
            <button className="relative p-2.5 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition">
          ...
          {stats.map((stat) => (
            <div key={stat.label} className="stat-card hover:shadow-md transition">
              <div className="flex items-center justify-between mb-3">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${stat.color === 'primary' ? 'bg-primary-100' :
                  stat.color === 'accent' ? 'bg-accent-100' :
                    stat.color === 'green' ? 'bg-green-100' : 'bg-amber-100'
                  }`}>
                  <stat.icon className={`w-5 h-5 ${stat.color === 'primary' ? 'text-primary-700' :
                    stat.color === 'accent' ? 'text-accent-600' :
                      stat.color === 'green' ? 'text-green-600' : 'text-amber-600'
                    }`} />
                </div>
                <span className="text-xs font-medium text-green-600 flex items-center gap-0.5 bg-green-50 px-2 py-0.5 rounded-full">
          ...
          <div>
            ...
            <div className="card space-y-3">
              {pendingActions.map((item, i) => (
                <Link key={i} href={item.link} className="flex items-start gap-3 p-3 rounded-lg hover:bg-gray-50 transition group">
                  <div className={`w-2 h-2 rounded-full mt-1.5 flex-shrink-0 ${item.urgency === 'high' ? 'bg-red-500' : item.urgency === 'medium' ? 'bg-amber-500' : 'bg-gray-300'
                    }`} />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-gray-700 group-hover:text-primary-700 transition">{item.label}</p>
                  </div>
                  <ArrowUpRight className="w-4 h-4 text-gray-300 group-hover:text-primary-500 transition flex-shrink-0" />
                </Link>
              ))}
            </div>
          </div>
